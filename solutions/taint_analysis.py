...
def flows(ast, unsafe_variables, implicit_variables=set()):
    ...
    for node in ast:
        match node['type']:
            case 'assert_statement': # If we have an assert statement, we check if any unsafe variables are used. 
            ...
            case 'if_statement': #how to handle else statements?
                ...
                if len(if_childs) > 2: # not an empty if
                    ...
                    res = set.union(res, flows(if_childs[2]['children'], unsafe_variables, new_implicit_vars)) 
