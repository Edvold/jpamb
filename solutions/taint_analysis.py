...
def flows(ast, unsafe_variables, seen_methods, implicit_variables=set()):
    res = set()
    for node in ast:
        ...
        match node['type']:
            ...
            case 'if_statement': #how to handle else statements?
                ...
                if len(if_childs) > 2: # not an empty if
                    ...
                    res = set.union(res, flows(if_childs[2]['children'], unsafe_variables, seen_methods, new_implicit_vars)) # not only if_childs[2]
                    if len(if_childs) >= 5: # with else (how about more if elses? maybe goes back to here)
                        res = set.union(res, flows(if_childs[4]['children'], unsafe_variables, seen_methods, new_implicit_vars))
            ...
            case 'while_statement':
                ...
                if len(while_childs) > 2: # not an empty while loop
                    ...
                    res = set.union(res, flows(while_childs[2]['children'], unsafe_variables, seen_methods, new_implicit_vars))
                #logger.debug(node['text'])
            case 'for_statement':
                ...
                if len(for_childs) > 4: # Thank you for being a normal human being (for loop with initialization, termination, increment and body)
                    ...
                    update_expr_flows = flows(for_childs[3]['children'], unsafe_variables, seen_methods, body_implicit_variables)
                    body_flows = flows(for_childs[4]['children'], unsafe_variables, seen_methods, body_implicit_variables) #two (or more) level flow from other expressions not added
            ...
            case 'expression_statement': #add update expression
                #logger.debug(node)
                res = set.union(res, flows(node['children'], unsafe_variables, seen_methods, implicit_variables))
            ...
            case 'enhanced_for_statement':
                ...
                method_flows = flows([for_each_childs[4]], unsafe_variables, seen_methods)
                ...
                body_flows = flows(for_each_childs[5]['children'], unsafe_variables, seen_methods, new_implicit_vars)
...
def method_flow(method_name, seen_methods): #only works for method in the same class
    ...
    flow_dict = flows(ast_body['children'], param_names, seen_methods)
